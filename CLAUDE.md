...
>
> **Guardrails are installed and mandatory:** `.githooks/` (pre-commit + commit-msg + pre-push) scans every outgoing change — diffs AND commit messages — for secret formats, personal info (non-allowlisted emails, phone numbers, server-identifying hostnames), forbidden file types, and private patterns from `~/.config/sms-repo-guard/patterns.txt` (names/domains/server IPs — kept outside the repo so the blocklist itself stays private). Commit identity is enforced against `~/.config/sms-repo-guard/identity.txt` (the approved public author), and pre-push re-checks every outgoing commit's author/committer **and re-scans the entire HEAD tree** (not just the diff, so a leak in an already-committed file is caught before it reaches the remote). After a fresh clone, re-enable with `git config core.hooksPath .githooks`. **Never bypass with `--no-verify`, weaken the scanner, or edit the private patterns file unless the owner explicitly asks** — a hook block is a hard stop: report it, don't work around it.
>
> **Always run the privacy audit before pushing:** `make privacy-audit` (or `scripts/privacy-audit.sh`) scans the whole working tree; `make privacy-audit-history` also scans full git history for already-published leaks. The pre-push hook runs the full-tree scan automatically, but run it yourself before any push to this repo — especially after pulling outside commits (e.g. GitHub Copilot/agent PRs), which the hook won't have vetted until you push. A couple of pre-mitigation leaks (a server IP and a cross-project admin email) remain in old `HANDOFF.md` history — removed from HEAD; run `make privacy-audit-history` to see them. Purging from history needs an owner-approved `git filter-repo` + force-push; the literal values are recorded only in the off-repo private memory, never here.
