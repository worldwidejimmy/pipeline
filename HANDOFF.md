...
- **Email** is via `send_email.py` using `SMTP_*` + `ADMIN_EMAIL` in `backend/.env` (currently reuses the Brevo relay; From/To point at `ops@example.com` until changed). Host scripts read `.env` live — no restart needed to change SMTP settings.
